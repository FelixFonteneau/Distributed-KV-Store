...
import se.kth.id2203.kvstore.OpCode.{Created, NotFound, NotImplemented, Ok, Updated, ValueDoesNotMatch}
    ...
    for (op <- h.getMinimalOperations) {
      ...
      if (result.isDefined && applyOperation(op, s) == result.get) {
        ...
        if (h.remove(op) && isLinearizable(h, s)) {
          ...
          h.update(oldHistory) // undo the operation and get the old state
        }
      } else {
